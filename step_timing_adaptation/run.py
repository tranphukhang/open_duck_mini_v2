...
if __package__:
    ...
    from .weight_shift_qp import (
        DoubleSupportHQPConfig,
        DoubleSupportWeightShiftController,
    )
...
WEIGHT_SHIFT_DURATION = 0.80
WEIGHT_SHIFT_HOLD_TIME = 0.20

# Final CoM target is placed this distance inward from the
# future stance-foot site origin.
WEIGHT_SHIFT_INNER_OFFSET = 0.010

WEIGHT_SHIFT_KP = 80.0
WEIGHT_SHIFT_KD = 18.0

# Desired final vertical-load distribution.
WEIGHT_SHIFT_LEFT_LOAD_FINAL = 0.90

WEIGHT_SHIFT_STATUS_PRINT_PERIOD = 0.10

# Readiness conditions before starting single support.
WEIGHT_SHIFT_POSITION_TOLERANCE = 0.008
WEIGHT_SHIFT_VELOCITY_TOLERANCE = 0.04
WEIGHT_SHIFT_MAX_TILT_DEG = 8.0
...
# ============================================================
# DOUBLE-SUPPORT WEIGHT-SHIFT PREPARATION
# ============================================================

def run_double_support_weight_shift(
    model,
    data,
    dynamics,
    controller,
    ...
    total_duration = (
        WEIGHT_SHIFT_DURATION
        +
        WEIGHT_SHIFT_HOLD_TIME
    )
    ...
    # Determine which side is LEFT directly from the model.
    #
    # Normally:
    #
    #     left_y > right_y
    ...
    # so lateral_sign = +1.
    #
    # The target is placed slightly inward from the left-foot
    # site origin so the CoM projection remains inside the
    # left-foot support polygon at the single-support switch.
    ...
    if abs(
        ...
        raise RuntimeError(
            "Could not determine left/right lateral direction."
        )
    ...
    # ========================================================
    # ROBOT WEIGHT / WRENCH REFERENCES
    # ========================================================
    ...
    print(
        f"shift distance = "
        f"{1000.0 * delta_com_y:+.2f} mm"
    )

    print(
        f"shift time     = "
        f"{WEIGHT_SHIFT_DURATION:.3f} s"
    )

    print(
        f"hold time      = "
        f"{WEIGHT_SHIFT_HOLD_TIME:.3f} s"
    )

    print(
        f"final left load reference = "
        f"{100.0 * WEIGHT_SHIFT_LEFT_LOAD_FINAL:.1f}%"
    )
    ...
    while True:
        ...
        if (
            ...
            raise RuntimeError(
                "Viewer closed during weight shift."
            )
        ...
        if (
            ...
            raise RuntimeError(
                "Emergency base tilt during weight shift."
            )
        ...
        desired_com_velocity_y = (
            delta_com_y
            *
            blend_rate
        )

        desired_com_feedforward_acceleration_y = (
            delta_com_y
            *
            blend_acceleration
        )
        ...
        # Ramp the desired load distribution from 50/50 toward
        # the future stance foot.

        left_load_fraction = (
            0.5
            +
            (
                WEIGHT_SHIFT_LEFT_LOAD_FINAL
                -
                0.5
            )
            *
        ...
        right_wrench_reference = np.array(
            [
                0.0,
                0.0,
                robot_weight
        ...
        if (
            ...
            solution = controller.solve_weight_shift(
                com_jdot_v_y=(
                    com_jdot_v[
                        1
                    ]
        ...
        if (
            ...
            *
            ...
            if last_solution is None:
                ...
                left_fz_fraction = np.nan
                right_fz_fraction = np.nan
                ...
                left_cop = compute_cop_from_wrench(
                    last_solution.left_wrench,
                    (
                        left.position[
                            2
                ...
                right_cop = compute_cop_from_wrench(
                    last_solution.right_wrench,
                    (
                        right.position[
                            2
            ...
            print(
                ...
                f" | load L/R="
                f"{100.0 * left_fz_fraction:5.1f}/"
                f"{100.0 * right_fz_fraction:5.1f}%"

                f" | CoP_L="
                f"({1000.0 * left_cop[0]:+.1f},"
                f"{1000.0 * left_cop[1]:+.1f})"

                f" | CoP_R="
                f"({1000.0 * right_cop[0]:+.1f},"
                f"{1000.0 * right_cop[1]:+.1f}) mm"
            ...
            next_print_elapsed += (
                WEIGHT_SHIFT_STATUS_PRINT_PERIOD
            )
    ...
    final_left_contact = has_geom_contact(
        data,
        left_geom_id,
        floor_geom_id,
    )

    final_right_contact = has_geom_contact(
        data,
        right_geom_id,
        floor_geom_id,
    )
    ...
    ready = (
        ...
        <=
        WEIGHT_SHIFT_POSITION_TOLERANCE
        and
        abs(
            final_velocity_y
        )
        <=
        WEIGHT_SHIFT_VELOCITY_TOLERANCE
        and
        final_tilt
        <=
        WEIGHT_SHIFT_MAX_TILT_DEG
        and
        final_left_contact
        and
        final_right_contact
    ...
    print(
        f"CoM-y error           = "
        f"{1000.0 * final_position_error:.3f} mm"
    )

    print(
        f"final CoM Vy          = "
        f"{final_velocity_y:+.6f} m/s"
    )
    ...
    if last_solution is not None:
        ...
        print(
            f"optimized left Fz     = "
            f"{last_solution.left_wrench[2]:+.6f} N"
        )

        print(
            f"optimized right Fz    = "
            f"{last_solution.right_wrench[2]:+.6f} N"
        )
    ...
    print(
        f"ready for LEFT single support = "
        f"{ready}"
    )
    ...
    if not ready:
        ...
        raise RuntimeError(
            "Weight-shift preparation did not reach a "
            "safe LEFT single-support initial state. "
            "Do not start the adaptive step yet."
        )
...
def main():
    ...
    weight_shift_config = DoubleSupportHQPConfig(
    ...
    if SHOW_VIEWER:
        ...
        with mujoco.viewer.launch_passive(
            ...
            run_double_support_weight_shift(
                model=model,
                data=data,
                dynamics=dynamics,
